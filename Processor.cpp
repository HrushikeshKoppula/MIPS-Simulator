...
void PROCESSOR::ID(){
    ...
    // I-Type
    for (int i=16;i<32;i++)   id.immediate[i-16]=Instruction[i];
    // J-Type
...
void PROCESSOR::EX(){
    ...
    if(idex.op==0b100011){// LW
        ...
        for(int i=16;i<32;i++)  idex.B[i]=idex.immediate[i-16];
        ALU::ADD(idex.A,idex.B,idex.zero,idex.Result,idex.Overflow);
        if(idex.Result[30]&&idex.Result[31])  std::cout<<"Address used is not Word Alligned\n";
    ...
    if(idex2.Overflow)  std::cout<<"Overflow occured\n";
}
void PROCESSOR::MEM(){
    ...
    if(idex2.op==0b100000){ //LB
        ...
        for(int i=0;i<8;i++)   idex2.Result[24+i]=DataMemory[effective_address+i];
    }
    if(idex2.op==0b101011){ //SW
        int effective_address = converter(idex2.Result);
        for(int i=0;i<32;i++)   DataMemory[effective_address+i]=REGISTERS::Registers[idex2.rt][i];
    }
    if(idex2.op==0b101000){ //SB
        int effective_address = converter(idex2.Result);
        for(int i=0;i<24;i++)   idex2.Result[i]=DataMemory[effective_address];
        for(int i=0;i<8;i++)   DataMemory[effective_address+i]=REGISTERS::Registers[idex2.rt][24+i];
    }
